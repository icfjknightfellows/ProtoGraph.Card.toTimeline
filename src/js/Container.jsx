    ...
    if (this.state.schemaJSON === undefined ){
      ...
      let assetIcons = events.map((element, pos) => {
        if(element.single_event.youtube_url){
          return (
            <svg id={element.timestamp} key={element.timestamp} className="protograph-asset-svg" dangerouslySetInnerHTML={{__html: "<image" + " x=" + (svgWidth/2 - 20) + " y=" + (that.getEventYCoord(element.single_event.timestamp_date, eventPoints) - 7) + " width=15" + " height=15" + " xlink:href='https://icfjknightfellows.github.io/ProtoGraph.Card.toTimeline/src/images/play.svg' />"}}/>

          );
        }
        if(element.single_event.photo){
          return (
            <svg id={element.timestamp} key={element.timestamp} className="protograph-asset-svg" dangerouslySetInnerHTML={{__html: "<image" + " x=" + (svgWidth/2 - 20) + " y=" + (that.getEventYCoord(element.single_event.timestamp_date, eventPoints) - 7) + " width=15" + " height=15" + " xlink:href='https://icfjknightfellows.github.io/ProtoGraph.Card.toTimeline/src/images/image.svg' />"}}/>
          );
        }
      });
  ...
  renderMobile() {
    if (this.state.schemaJSON === undefined ){
      ...
      let assetIcons = events.map((element, pos) => {
        if(element.single_event.youtube_url){
          return (
            <svg id={element.timestamp} key={element.timestamp} className="protograph-asset-svg" dangerouslySetInnerHTML={{__html: "<image" + " x=" + (svgWidth/2 - 25) + " y=" + (that.getEventYCoord(element.single_event.timestamp_date, eventPoints) - 7) + " width=15" + " height=15" + " xlink:href='https://icfjknightfellows.github.io/ProtoGraph.Card.toTimeline/src/images/play.svg' />"}}/>
